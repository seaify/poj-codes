...
void inline insert(int c,int x,int y,int k)//更新一个区间
{
 int l,r;
 if(k==-1)
    return ;
...
void inline sum(int x,int y,int k)
{
 int l,r,mid;
 l=tree[k].beg;r=tree[k].end;
 if(k==-1)
    return ;
 if(l<=x&&y<=r&&tree[k].c!=-1)//
 {
	flag[tree[k].c]=1;
    return ;
 }
...
int main()
{
  ...
  while(T--)
  {
	...
	init(0,temp);//建树。。。。
	for(i=1;i<=n;++i)   
      insert(i,upper_bound(p,p+temp+1,x[i])-p-1,upper_bound(p,p+temp+1,y[i])-p-1,1);
