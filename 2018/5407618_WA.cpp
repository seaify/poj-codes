...
//	freopen("out1.txt","w+",stdout);
	...
	while(EOF!=scanf("%d %d",&N,&F))
	{
	    ...
	    if((1000*(sum[F+1]-sum[1]))/F>opt)
			opt=(1000*(sum[F+1]-sum[1]))/F; /////为判断出单调性，必须先压入2个点。。。
		for(i=F+2;i<=N;++i)
		{
			...
			if(q.size()==1)
			{
				last=1000*(cnt.y-q.back().y)/(cnt.x-q.back().x);
				continue;
			}
			t=q.back();    ////此时必存在2个元素
			q.pop_back();
			s=q.back();
			while(1000*(cnt.y-t.y)/(cnt.x-t.x)<1000*(cnt.y-s.y)/(cnt.x-s.x))  ////保证至少2个元素...,last不断的扩大。。直到碰到一个比它小或等于的
			{
			   t=q.back();
			   q.pop_back();
			   if(q.empty())
